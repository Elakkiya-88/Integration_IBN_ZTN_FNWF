...
bandwidths = ["50K", "100K", "1M", "5M", "1G"]
#bandwidths = ["50K", "100K", "200K", "300K", "400K", "500K", "1M", "5M", "10M", "20M", "50M", "100M", "500M", "1G"]
#extracted_bandwidth = '200K' #Extracted bandwidth
#bandwidths = ['300K', '200K','50K', '400K'] # '100K','550K', '110K', '75K','250K', '150K', '350K', '150K', '450K', '500K', '100M', '150M', '50M'] #"600K","700K","800K" Bandwidths in Kbps
# bandwidths = []

# # # Add Kbps bandwidths (50K to 950K in steps of 50)
# for kbps in range(50, 1000, 50):  # 50, 100, ..., 950
#  bandwidths.append(f"{kbps}K")

# # # Add Mbps bandwidths (1M to 50M in steps of 1)
# for mbps in range(1, 51):  # 1M, 2M, ..., 50M
#  bandwidths.append(f"{mbps}M")

# # # Total number of bandwidths
# print(f"Generated {len(bandwidths)} bandwidths.")
# print(bandwidths)
...
def apply_congestion():
    subprocess.run([
        ...
        "tbf", "rate", "100kbit", "burst", "30kbit", "latency", "300ms"
...
#         writer = csv.writer(f)
#         writer.writerow(["Bandwidth", "Measured (No Congestion) [Kbps]", "Measured (With Congestion) [Kbps]"])
#         for row in data:
#             writer.writerow(row)
...
def write_combined_csv(data, filename="combined_data.csv"):
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Bandwidth", "Unit", "Measured (No Congestion) [Kbps]", "Measured (With Congestion) [Kbps]"])
        for row in data:
            writer.writerow(row)
...
#         writer = csv.writer(f)
#         writer.writerow(["Bandwidth", "Unit", "Measured (No Congestion) [Kbps]", "Measured (With Congestion) [Kbps]"])
#         for bw, unit, no_cong, with_cong in data:
#             writer.writerow([bw, unit, no_cong, with_cong])

        # for bandwidth, unit, without cong, with cong in data:
        #     writer.writerow([bw, unit, no_cong, with_cong])
...
def categorize_bandwidth(bandwidth):
    match = re.match(r"\d+(K|M|G)", bandwidth)
...
def main():
    ...
    # for bw in bandwidths:
        print(f"Testing {bandwidth} without congestion...")
        #remove_congestion()
        out1 = run_iperf_test(bandwidth)
        parsed1 = parse_bandwidth_data(out1)
        val1 = np.mean(parsed1) if parsed1 else 0.0
        print("without cong:", val1)
    
        print(f"Testing {bandwidth} with congestion...")
        apply_congestion()
        out2 = run_iperf_test(bandwidth)
        parsed2 = parse_bandwidth_data(out2)
        val2 = np.mean(parsed2) if parsed2 else 0.0
        print("with cong:", val2)
        #results.append((bandwidth, unit, val1, val2))
        remove_congestion()
    
        unit = categorize_bandwidth(bandwidth)
        results.append((bandwidth, unit, val1, val2))
        
    # ---- Output ----
    write_combined_csv(results, "iperf_bandwidth_comparison_with_congestion.csv")
    print("\nComparison (Kbps):")
    for bandwidth, unit, no_cong, with_cong in results:
        print(f"{bandwidth} ({unit}): Without Congestion = {no_cong:.2f}, With Congestion = {with_cong:.2f}")
    
        write_combined_csv(results, "iperf_bandwidth_comparison_all.csv")
   
        # Predict bandwidth using the BiLSTM model
        predicted_bandwidth = parse_bandwidth_data_with_prediction(output, bilstm_model, scaler, sequence_length)

        # Combine time, predicted bandwidth, and actual bandwidth (from iPerf)
        for predicted_bw in predicted_bandwidth:
            combined_data.append([current_time, predicted_bw, bandwidth])
            current_time += 1

    # Write the combined data to CSV# # Reward calculation function
    # def calculate_reward(target_bandwidth, achieved_bandwidth):
    #     return -abs(target_bandwidth - achieved_bandwidth)
...
#    X_train, y_train = X[:split_index], y[:split_index]
    ...
    '300': 300, '200': 200, '490': 490, '420': 420
    }
...
# import re
    ...
    # thresholds = {
    #     "In-Distribution (250 Kbps)": extract_bandwidth_from_nile(nile_intent_in),
    ...
    #     df['Group'] = (df['Time Step (UE State)'] - 1) // 5
    ...
    #bar1 = ax.bar(x - width/2, ep15_match, width, label='Ep15 Match (%)', color='green')
    #bar2 = ax.bar(x - width/2, ep15_nomatch, width, bottom=ep15_match, label='Ep15 No Match (%)', color='lightgreen')
    #bar3 = ax.bar(x + width/2, ep39_match, width, label='Ep39 Match (%)', color='purple')
    #bar4 = ax.bar(x + width/2, ep39_nomatch, width, bottom=ep39_match, label='Ep39 No Match (%)', color='plum')
    
    #ax.set_ylabel('Percentage (%)')
    #ax.set_title('Match vs No Match with Bandwidth Thresholds')
    #ax.set_xticks(x)
    #ax.set_xticklabels(labels, rotation=15)
    #ax.legend()
    
    #for i in range(len(labels)):
    #    ax.text(x[i] - width/2, ep15_match[i] + 1, f"{ep15_match[i]:.1f}%", ha='center', va='bottom', fontsize=9)
    #    ax.text(x[i] + width/2, ep39_match[i] + 1, f"{ep39_match[i]:.1f}%", ha='center', va='bottom', fontsize=9)
    
    #plt.tight_layout()
    #plt.savefig("match_bar_chart.png")
    #plt.show()
